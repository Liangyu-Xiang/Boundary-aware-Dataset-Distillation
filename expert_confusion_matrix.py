...
@torch.no_grad()
def update_confusion_matrix(cm: torch.Tensor, preds: torch.Tensor, targets: torch.Tensor, num_classes: int):
    """
    cm: [C, C] where rows are GT, cols are Pred.
    preds/targets: [B] int64
    """
    ...
    idx = targets * num_classes + preds  # [B]
...
def main(args):
    ...
    if rank == 0:
        ...
        cm_dir = f"{experiment_dir}/confusion"
    ...
    for epoch in range(1):
        ...
        for x_dit, x_expert, ry, y in data_iter: 
            ...
            with torch.no_grad():
                ...
                expert_pred = expert_logits.argmax(dim=1)   # [B]
    ...
    if rank == 0:
        cm_dir = os.path.join(experiment_dir, "confusion")
        os.makedirs(cm_dir, exist_ok=True)

        cm_np = confusion_matrix.cpu().numpy()

        # raw counts
        np.save(os.path.join(cm_dir, "confusion.npy"), cm_np)
        np.savetxt(os.path.join(cm_dir, "confusion.csv"),
                   cm_np, fmt="%d", delimiter=",")

        # normalized (per GT class)
        cm_norm = cm_np / (cm_np.sum(axis=1, keepdims=True) + 1e-12)
        np.save(os.path.join(cm_dir, "confusion_norm.npy"), cm_norm)
        np.savetxt(os.path.join(cm_dir, "confusion_norm.csv"),
                   cm_norm, fmt="%.6f", delimiter=",")

        logger.info(f"Confusion matrix saved to {cm_dir}")
