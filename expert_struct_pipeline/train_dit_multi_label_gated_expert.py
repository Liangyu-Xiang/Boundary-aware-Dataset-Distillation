...
from expert_struct_pipeline.expert_loader import load_expert_model
    ...
    for epoch in range(args.epochs):
        ...
        for x_dit, x_expert, ry, y in loader: 
            ...
            with torch.no_grad():
                ...
                # -------- multi-label gate --------
                # only boundary samples trigger multi-label
                use_multi = top2_prob > 0.1                          # [B] bool
                ...
                # -------- weight for secondary label --------
                # normalized weight, but clipped to avoid over-dominance
                w2 = top2_prob / (top1_prob + top2_prob + 1e-8)
                w2 = torch.clamp(w2, max=0.5)                       # <= 25%
                w1 = 1.0 - w2
            ...
            if use_multi.any():
                ...
                loss_dict2 = diffusion.training_losses(model, x, t, dict(y=y_secondary))
                loss2[use_multi] = loss_dict2["loss"][use_multi]
            
            # =========================
            # 4. Final loss (gated)
            # =========================
            per_sample_loss = loss1
            per_sample_loss[use_multi] = (
                w1[use_multi] * loss1[use_multi] +
                w2[use_multi] * loss2[use_multi]
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expert-model", type=str, default="resnet18",
                        help="Expert architecture name. Torchvision model names are supported when --expert-ckpt is set.")
    parser.add_argument("--expert-ckpt", type=str, default="",
                        help="Checkpoint for the expert used to guide DiT fine-tuning.")
    args = parser.parse_args()
    if args.expert_ckpt == "":
        args.expert_ckpt = None
    main(args)
