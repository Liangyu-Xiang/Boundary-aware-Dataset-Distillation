...
def main(args):
    ...
    confusion = np.load("/data/mmc_lyxiang/DD/MinimaxDiffusion/logs/confusion/1K_confusion/004-DiT-XL-2-1K_confusion/confusion/confusion.npy")
    confusion = torch.from_numpy(confusion).long().to(device)
    confusion.fill_diagonal_(0)
    topk_confused = torch.topk(confusion, k=10, dim=1).indices
    print(f"[INFO] Loaded confusion matrix.")
    ...
    for class_label, sel_class in zip(class_labels, sel_classes):
        ...
        for shift in tqdm(range(math.ceil(args.num_samples / batch_size))):
            ...
            confused_candidates = topk_confused[class_label]
            rand_idx = torch.randint(
                low=0,
                high=confused_candidates.numel(),
                size=(candidate_batch_size,),
                device=device
            )
            random_labels = confused_candidates[rand_idx]
            ...
            label_weight = 0.2 * weight_id + 0.8
            # label_weight = 0.5
            ...
            final_random_labels = random_labels
