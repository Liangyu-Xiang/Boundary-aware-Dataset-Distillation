...
def main(args):
    ...
    for class_label, sel_class in zip(class_labels, sel_classes):
        ...
        for shift in tqdm(range(args.num_samples // batch_size)):
            ...
            y_total = torch.tensor(woof_indices, device=device)
            class_label_index = torch.nonzero(y_total == class_label).item()
            y_hat = torch.cat([y_total[class_label_index:class_label_index+1], y_total[:class_label_index], y_total[class_label_index+1:]])

            y = torch.cat([y_hat, y_null], 0)
            z = z.repeat(y.size(0), 1, 1, 1)
            cfg_total_scale = [args.cfg_scale] + [1.0] * (len(y_total) -1)
            model_kwargs = dict(y=y, cfg_scale=cfg_total_scale)

            # Sample images:
            samples = diffusion.p_sample_loop(
                model.forward_with_cfg, z.shape, z, clip_denoised=False, model_kwargs=model_kwargs, progress=False, device=device
            )
            samples = samples[:1]  # Remove null class samples
