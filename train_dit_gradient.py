    ...
    for epoch in range(args.epochs):
        ...
        for x_dit, x_expert, ry, y in loader: 
            ...
            with torch.no_grad():
                ...
                tau = torch.quantile(grad_proxy, 0.7)
                uncertain_mask = grad_proxy > tau                  # [B] bool
                ...
                y_secondary = torch.full_like(ry_t, 1000)          # [B]

                # case 1: expert prediction is WRONG → use top-1
                y_secondary[expert_wrong] = idx_to_y[top1_cls[expert_wrong]]

                # case 2: expert prediction is CORRECT & UNCERTAIN → use top-2
                use_top2_mask = (~expert_wrong) & uncertain_mask
                y_secondary[use_top2_mask] = idx_to_y[top2_cls[use_top2_mask]]


                # stack into [2， B]
                y_two = torch.stack([y, y_secondary], dim=0)  # [2, B]

                # optional sharpening (analogous to beta in entropy)
                beta = 1.0
                weight = torch.exp(beta * grad_proxy)

                # normalize for stability
                # weight = weight / (weight.mean() + 1e-8)
                weight = weight.detach()      # [B]

            with torch.no_grad():
                # Map input images to latent space + normalize latents:
                x = vae.encode(x_dit).latent_dist.sample().mul_(0.18215)
            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y=y_two)
