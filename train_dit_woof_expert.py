...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for x, ry, y in loader:
            ...
            with torch.no_grad():
                logits_x = expert_model(x)
                probs_x = F.softmax(logits_x, dim=-1)
                data_u_x = -torch.sum(probs_x * torch.log(probs_x + 1e-8), dim=-1)
            ...
            preded_x = vae.decode(pseudo_embeddings / 0.18215).sample
            preded_logits_x = expert_model(expert_transform(preded_x))
            preded_probs_x = F.softmax(preded_logits_x, dim=-1)
            preded_data_u = -torch.sum(preded_probs_x * torch.log(preded_probs_x + 1e-8), dim=-1)
            delta_data_u = (data_u_x - preded_data_u).abs()
            loss = torch.sigmoid(1.0 * (delta_data_u - 0)) * loss_dict["loss"]
            loss = loss.mean()
            ...
            if args.condense:
                ...
                for c in ry_set:
                    ...
                    # logits = expert_model(expert_transform(input)) / temperature
                ...
                all_loss = loss + pos_match_loss + neg_match_loss + 0.005 * classifier_loss
            else:
                all_loss = loss
            ...
            if train_steps % args.log_every == 0:
                ...
                logger.info(f"(step={train_steps:07d}) Train Loss: {avg_loss:.4f} {avg_loss_pos:.4f} {avg_loss_neg:.4f} {avg_loss_alea:.4f}, Train Steps/Sec: {steps_per_sec:.2f}")
