...
def main(args, logger, repeat=1):
    ...
    for i in range(repeat):
        ...
        args_teacher = args
        args_teacher.net_type = 'resnet'
        args_teacher.depth = 18
        model_teacher =  define_model(args_teacher, nclass)
...
def train_epoch(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if r < args.mix_p and mixup == 'cut':
            ...
            output = model(input)
            loss = 0.0 * (criterion(output, target) * ratio + criterion(output, target_b) * (1. - ratio))
            if model_teacher is not None:
                with torch.no_grad():
                    teacher_output = model_teacher(input)
                loss += 1.0 * (distillation_loss(output, teacher_output, 2) * ratio + distillation_loss(output, teacher_output, 2) * (1. - ratio))
