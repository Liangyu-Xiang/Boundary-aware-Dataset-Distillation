...
class MultiTargetCrossEntropy(nn.Module):
    """Cross-entropy that supports multiple target indices per sample."""
    ...
    def forward(self, input, target):
        ...
        if target.dim() != 2:
            raise ValueError(f"Unsupported target shape for multi-target CE: {target.shape}")

        # target is a list of label indices per sample; assign 0.9/0.1 to primary/secondary.
        if target.size(1) < 2:
            raise ValueError(f"Expected at least 2 labels per sample, got {target.shape}")

        target_probs = torch.zeros_like(input, dtype=input.dtype)
        primary = target[:, 0]
        secondary = target[:, 1]
        target_probs.scatter_(1, primary.unsqueeze(1), 0.8)
        target_probs.scatter_(1, secondary.unsqueeze(1), 0.2)
        log_probs = F.log_softmax(input, dim=1)
        loss = -(target_probs * log_probs).sum(dim=1).mean()
        return loss


def multi_target_accuracy(output, target, topk=(1, )):
    """Compute accuracy where each sample may have multiple correct labels."""
    if target.dim() > 1 and target.size(-1) == 1:
        target = target.squeeze(-1)
    ...
    for k in topk:
        pred_k = pred[:, :k]
        matches = (pred_k.unsqueeze(-1) == target.unsqueeze(1)).any(dim=2).any(dim=1)
        correct_k = matches.float().sum() * (100.0 / batch_size)
...
def train(args, model, train_loader, val_loader, plotter=None, logger=None):
    criterion = MultiTargetCrossEntropy().cuda()
...
def train_epoch(args,
    ...
    for i, batch in enumerate(train_loader):
        if isinstance(batch, (list, tuple)):
            ...
            raise ValueError("Unexpected batch structure from data loader.")
        ...
        if r < args.mix_p and mixup == 'cut':
            ...
            if target.dim() == 1:
                ...
                rand_index = torch.randperm(target.size(0), device=target.device)
        ...
        acc1, acc5 = multi_target_accuracy(output.data, target, topk=(1, 5))
