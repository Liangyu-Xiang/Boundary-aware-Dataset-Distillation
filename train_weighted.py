...
from misc.utils import random_indices, rand_bbox, AverageMeter, accuracy, get_time, Plotter, load_model
...
def main(args, logger, repeat=1):
    ...
    for i in range(repeat):
        ...
        best_acc, acc = train(args, model, teacher, train_loader, val_loader, plotter, logger)
...
def train(args, model, teacher, train_loader, val_loader, plotter=None, logger=None):
...
def compute_sample_weight(teacher_output, target, mode="entropy", eps=1e-8, temperature=1.0, epoch=None):
    with torch.no_grad():
        p = torch.softmax(teacher_output, dim=1)
        uniform_p = torch.full_like(p, 1.0 / p.size(1))
        pc = p.gather(1, target.view(-1,1)).squeeze(1)
        uniform_pc = uniform_p.gather(1, target.view(-1,1)).squeeze(1)

        if mode == "conf":
            s = 1.0 - pc
            uniform_s = 1.0 - uniform_pc
        elif mode == "nll":
            s = -torch.log(pc.clamp_min(eps))
            uniform_s = -torch.log(uniform_pc.clamp_min(eps))
        elif mode == "entropy":
            s = -(p * (p.clamp_min(eps)).log()).sum(dim=1)
            uniform_s = -(uniform_p * (uniform_p.clamp_min(eps)).log()).sum(dim=1)
        elif mode == "chi":
            s = torch.sqrt((1.0 - pc).clamp_min(0.0) / pc.clamp_min(eps))
            uniform_s = torch.sqrt((1.0 - uniform_pc).clamp_min(0.0) / uniform_pc.clamp_min(eps))
        else:
        ...
        # uniform_s = uniform_s / (uniform_s.mean().detach() + eps)
        s = s / uniform_s.detach()
        if epoch:
            s = uniform_s + (s - uniform_s) * max(0.0, (args.epochs - epoch)) / args.epochs
...
def train_epoch(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        weights = compute_sample_weight(teacher_output,target)
